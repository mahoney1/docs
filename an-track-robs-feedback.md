...
---
~~ There is at least one occurence of VSCode - without the space - in this doc 
...
~~ I would check that you are happy with the wording and punctuation of this paragraph.  Also - do you need a quick side note on SHEEPGOAT?
...
1. You will need to have the following installed in order to proceed:  ~~Can I expect this to work on Windows/Mac/Linux ?

* (Node v8.x or greater and npm v5.x or greater)[https://nodejs.org/en/download/]  ~~ need to install typescript too !!
* (Yeoman (yo) v2.x)[https://yeoman.io/]  ~~ Do I really need to install yo ?
...
* Yarn - install yarn as follows in your $HOME directory:
     ...
     * `npm install yarn`   ~~priv user?  Any particular folder to install in, or use -g option ?
...
6. In VSCode Explorer, choose File > Open Folder, and navigate to the `animaltracking` folder in your cloned repo - then select the `contracts` folder, eg. navigating to the `$HOME/animaltracking/contract` directory. The `contract` folder must be your top-level project folder in VSCode  ~~Explorer before proceeding
...
~~ Is this Step 10 in the right place ??
...
`EventClient.ts`  ~~BackTick error in this block  AND ~~ do you need to edit Client.ts too ?  ~~Should you point us to the folder where we can find these files
'EventClient_IBP.ts`
`QueryClient.ts`
`QueryClient_IBP.ts`
`QueryEvents.ts`
...
Successful completion of these pre-reqs  ~~ and preparatory steps, is the basis from which this tutorial can now proceed. 

## Estimated time

After the prerequisites  ~~ and preparatory steps are completed, this should take approximately *60 minutes* to complete.
...
`cp $HOME/dash/animaltracking/react/icons/* ../public/demo/icons` ~~the target folder icons did not exist for me
...
`ln -s ledger $HOME/dash/animaltracking/typescript/client/lib`  ~~remember to check/fix this
